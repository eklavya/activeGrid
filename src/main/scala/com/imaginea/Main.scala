        ...
        put {
          entity(as[Site1]) { site =>
            val buildSite = Future {
              ...
              logger.info(s"Time for AWS : ${new Date()} savedSite ${savedSite.getId}")
              site1
            }
      ...
      siteId =>
        get {
          val siteObj = Future {
            ...
            if(cachedSite.contains(siteId)) cachedSite.get(siteId) else Site1.fromNeo4jGraph(siteId)
          }
          onComplete(siteObj) {
            case Success(response) => complete(StatusCodes.OK, response)
            case Failure(exception) =>
          ...
          mayBeSite match {
            case Some(site) =>
              val keyPairs = site.instances.flatMap { instance =>
                instance.sshAccessInfo.flatMap(x => x.keyPair)
              }
