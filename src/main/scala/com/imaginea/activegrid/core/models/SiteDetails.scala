...
/**
  * Created by sivag on 25/11/16.
  */
case class SiteDetails(override val id: Option[Long],
                 siteName: String,
...
object SiteDetails {
  ...
  def apply(id: Long): SiteDetails = {
    SiteDetails(Some(id), "test", List.empty[Instance], List.empty[ReservedInstanceDetails],
  ...
  def fromNeo4jGraph(nodeId: Long): Option[SiteDetails] = {
    ...
    mayBeNode match {
      case Some(node) =>
        ...
        Some(SiteDetails(Some(nodeId), siteName, instances, reservedInstance, siteFilters, loadBalancers, scalingGroups, instanceGroups))
      case None =>
  ...
  implicit class RichSiteDetails(siteDetails: SiteDetails) extends Neo4jRep[SiteDetails] {

    override def toNeo4jGraph(entity: SiteDetails): Node = {
      val label = "Site1"
      val mapPrimitives = Map("siteName" -> entity.siteName)
      val node = Neo4jRepository.saveEntity[SiteDetails](label, entity.id, mapPrimitives)
      val relationship_inst = "HAS_Instance"
    ...
    override def fromNeo4jGraph(id: Long): Option[SiteDetails] = {
      SiteDetails.fromNeo4jGraph(id)
    }
  }
