...
object ScalingGroup {
  ...
  def fromNeo4jGraph(id: Long): Option[ScalingGroup] = {
    ...
    mayBeNode match {
      case Some(node) =>
        val map = Neo4jRepository.getProperties(node, "name", "launchConfigurationName", "status",
          "availabilityZones", "instanceIds", "loadBalancerNames", "desiredCapacity", "maxCapacity", "minCapacity")
