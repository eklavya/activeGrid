          ...
          val keyValAndSnapshot = node.getRelationships.foldLeft((List.empty[KeyValueInfo], SnapshotInfo.appply(1))) {
            (result, relationship) =>
              ...
              relationship.getType.name match {
                case `volumeInfoTagRelation` => val keyValueInfo = KeyValueInfo.fromNeo4jGraph(childNode.getId)
                  if (keyValueInfo.nonEmpty) (keyValueInfo.get :: result._1, result._2) else result
                case `volumeInfoSnapshotInfoRelation` => val snapshotInfoObj = SnapshotInfo.fromNeo4jGraph(childNode.getId)
