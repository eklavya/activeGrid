    ...
    val instances: List[Instance] = awsInstancesResult.map {
        awsInstance =>
          ...
          val instance = new Instance(None,
            ...
            Some(new StorageInfo(None, 0D, AWSInstanceType.toAWSInstanceType(awsInstance.getInstanceType).ramSize)),
            Some(new StorageInfo(None, 0D, AWSInstanceType.toAWSInstanceType(awsInstance.getInstanceType).rootPartitionSize)),
            awsInstance.getTags.map(tag => new KeyValueInfo(None , tag.getKey , tag.getValue)).toList,
            None,
            List(),
            List(),
            Set(),
            Some(imageInfo),
            List(new InstanceUser(Some(1), "username", List("keys")))
          )
