...
object SiteFilter {
  ...
  val siteFilterAccountInfoRel = "HAS_ACCOUNT_INFO"
  val siteFilterFiltersRel = "HAS_FILTERS"
  ...
  implicit class SiteFilterImpl(siteFilter: SiteFilter) extends Neo4jRep[SiteFilter] {
    override def toNeo4jGraph(entity: SiteFilter): Node = {
      ...
      Neo4jRepository.createRelation(siteFilterAccountInfoRel, node, accountInfoNode)
      entity.filters.foreach {
        filter =>
          val filterNode = filter.toNeo4jGraph(filter)
          Neo4jRepository.createRelation(siteFilterFiltersRel, node, filterNode)
      }
      node
    }
      ...
      maybeNode match {
        case Some(node) =>
          if (Neo4jRepository.hasLabel(node, siteFilterLabel)) {
            ...
            val childNodeIdsAccountInfos: List[Long] = Neo4jRepository.getChildNodeIds(nodeId, siteFilterAccountInfoRel)
            val accountInfos: List[AccountInfo] = childNodeIdsAccountInfos.flatMap { childId =>
              AccountInfo.fromNeo4jGraph(childId)
            }

            val childNodeIdsFilter: List[Long] = Neo4jRepository.getChildNodeIds(nodeId, siteFilterFiltersRel)
            val filters: List[Filter] = childNodeIdsFilter.flatMap { childId =>
