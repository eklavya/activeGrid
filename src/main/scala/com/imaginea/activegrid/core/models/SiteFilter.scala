    ...
    override def toNeo4jGraph(entity: SiteFilter): Node = {
      ...
      repository.withTx {
        neo =>
          ...
          repository.createRelation(siteFilterAccountInfoRel, node, accountInfoNode)
          entity.filters.foreach {
            filter =>
              val filterNode = filter.toNeo4jGraph(filter)
              repository.createRelation(siteFilterFiltersRel, node, filterNode)
          }
          node
      }
    }
            ...
            val accountAndFilter = node.getRelationships.foldLeft((AccountInfo.apply(1), List[Filter]())) {
              (result, relationship) =>
                ...
                relationship.getType.name match {
                  case `siteFilterAccountInfoRel` => val accountInfo = AccountInfo.fromNeo4jGraph(childNode.getId)
                    if (accountInfo.nonEmpty) (accountInfo.get, result._2) else result
                  case `siteFilterFiltersRel` => val filter = Filter.fromNeo4jGraph(childNode.getId)
