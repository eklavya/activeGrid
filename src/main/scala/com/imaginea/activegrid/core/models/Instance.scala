    ...
    mayBeNode match {
      case Some(node) =>
        ...
        val memoryInfo: Option[StorageInfo] =
          Neo4jRepository.getChildNodeId(nodeId, relationship_info1).flatMap(id => StorageInfo.fromNeo4jGraph(id))

        val relationship_info2 = "HAS_storageInfo2"
        val rootDiskInfo: Option[StorageInfo] =
          Neo4jRepository.getChildNodeId(nodeId, relationship_info2).flatMap(id => StorageInfo.fromNeo4jGraph(id))

        val relationship_ssh = "HAS_sshAccessInfo"
        val sshAccessInfo: Option[SSHAccessInfo] =
          Neo4jRepository.getChildNodeId(nodeId, relationship_ssh).flatMap(id => SSHAccessInfo.fromNeo4jGraph(id))

        val relationship_image = "HAS_imageInfo"
        val imageInfo: Option[ImageInfo] =
          Neo4jRepository.getChildNodeId(nodeId, relationship_image).flatMap(id => ImageInfo.fromNeo4jGraph(id))
