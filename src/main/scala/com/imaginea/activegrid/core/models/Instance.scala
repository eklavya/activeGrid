...
object Instance {
  ...
  def apply(name: String, tags: List[KeyValueInfo], processes: Set[ProcessInfo]): Instance =
    Instance(None, None, name, None, None, None, None, None, None, None, None, tags, None, List.empty[InstanceConnection],
      List.empty[InstanceConnection], processes, None, List.empty[InstanceUser], None, None, None, None, None, None, None,
      None, List.empty, List.empty, reservedInstance = false, None)

  def apply(name: String): Instance =
    Instance(None, None, name, None, None, None, None, None, None, None, None, List.empty[KeyValueInfo], None, List.empty[InstanceConnection],
      List.empty[InstanceConnection], Set.empty[ProcessInfo], None, List.empty[InstanceUser], None, None, None, None, None, None, None, None,
      List.empty, List.empty, reservedInstance = false, None)
  ...
  def fromNeo4jGraph(nodeId: Long): Option[Instance] = {
    ...
    mayBeNode match {
      case Some(node) =>
        val map = Neo4jRepository.getProperties(node, "instanceId", "name", "state", "instanceType", "platform", "architecture",
          "publicDnsName", "availabilityZone", "privateDnsName", "privateIpAddress", "publicIpAddress", "elasticIP", "monitoring",
          "rootDeviceType", "reservedInstance", "region")
        ...
        val launchTime: Option[Long] = Some(defaultLaunchTime)
        ...
        Some(Instance(Some(nodeId), instanceId, name, state, instanceType, platform, architecture, publicDnsName, launchTime, memoryInfo, rootDiskInfo,
          tags, sshAccessInfo, liveConnections, estimatedConnections, processes, imageInfo, existingUsers, None, availabilityZone, privateDnsName,
  ...
  implicit class InstanceImpl(instance: Instance) extends Neo4jRep[Instance] {
    ...
    override def toNeo4jGraph(entity: Instance): Node = {
      ...
      val mapPrimitives = Map("instanceId" -> entity.instanceId,
        "name" -> entity.name, "state" -> entity.state, "instanceType" -> entity.instanceType,
        "platform" -> entity.platform, "architecture" -> entity.architecture,
        "publicDnsName" -> entity.publicDnsName, "launchTime" -> entity.launchTime,
        "availabilityZone" -> entity.availabilityZone, "privateDnsName" -> entity.privateDnsName,
        "privateIpAddress" -> entity.privateIpAddress, "publicIpAddress" -> entity.publicIpAddress,
        "elasticIP" -> entity.elasticIP, "monitoring" -> entity.monitoring,
        "rootDeviceType" -> entity.rootDeviceType, "reservedInstance" -> entity.reservedInstance, "region" -> entity.region
      )
      ...
      entity.memoryInfo match {
        case Some(mInfo) =>
          ...
          Neo4jRepository.setGraphRelationship(node, memoryInfoNode, relationship_info1)
      ...
      entity.rootDiskInfo match {
        case Some(rInfo) =>
          ...
          Neo4jRepository.setGraphRelationship(node, rootDiskInfoNode, relationship_info2)
      ...
      entity.tags.foreach { tag =>
        ...
        Neo4jRepository.setGraphRelationship(node, tagNode, relationship_keyValueInfo)
      }
      ...
      entity.existingUsers.foreach { existingUser =>
        ...
        Neo4jRepository.setGraphRelationship(node, existingUserNode, relationship_user)
      }
      ...
      entity.blockDeviceMappings.foreach { blockDeviceMapping =>
        ...
        Neo4jRepository.setGraphRelationship(node, blockDeviceNode, relationship_blockDevice)
      }
