...
object ImageInfo {
  ...
  implicit class ImageInfoImpl(imageInfo: ImageInfo) extends Neo4jRep[ImageInfo] {
    ...
    override def fromNeo4jGraph(nodeId: Long): ImageInfo = {
      ...
      ImageInfo(Some(nodeId),imageId,state, ownerId, publicValue, architecture, imageType,platform, imageOwnerAlias, name, description, rootDeviceType, rootDeviceName, version)
